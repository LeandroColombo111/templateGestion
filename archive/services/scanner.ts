...
const suspiciousTlds = [".zip", ".mov", ".xyz", ".top", ".click", ".work"];
const urlShorteners = [
  "bit.ly",
  "t.co",
  "tinyurl.com",
...
export function evaluateLinks(urls: string[]): {
  ...
  return { links, hasShortenedUrl, suspiciousCount };
}
...
export function scanEmail(input: {
  ...
  const shortened = input.links.some((link) => {
    ...
    return urlShorteners.includes(domain);
  });
  if (shortened) {
    reasons.push({
      code: "URL_SHORTENER",
  ...
  const suspiciousLinks = input.links.filter((link) => link.is_suspicious);
  ...
  const spf = input.headers.spf?.toLowerCase();
  const dkim = input.headers.dkim?.toLowerCase();
  const dmarc = input.headers.dmarc?.toLowerCase();
  if (spf && spf !== "pass") {
    reasons.push({
      code: "SPF_FAIL",
      message: "SPF validation failed or is missing.",
      weight: 8
    });
  }
  if (dkim && dkim !== "pass") {
    reasons.push({
      code: "DKIM_FAIL",
      message: "DKIM validation failed or is missing.",
      weight: 8
    });
  }
  if (dmarc && dmarc !== "pass") {
    reasons.push({
      code: "DMARC_FAIL",
      message: "DMARC alignment failed or is missing.",
      weight: 10
    });
  }
  ...
  const signals: ScanSignals = {
    ...
    authFailures: Boolean(
      (spf && spf !== "pass") || (dkim && dkim !== "pass") || (dmarc && dmarc !== "pass")
    )
  };
